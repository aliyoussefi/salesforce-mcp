...
export interface Tokens { accessToken?: string; refreshToken?: string; expiresAt?: number; clientSecret?: string; }
export interface TokenProvider { get(server: string): Promise<Tokens | undefined>; set(server: string, tokens: Tokens): Promise<void>; }
...
export class FileTokenStore implements TokenProvider {
  constructor(private readonly file: string | Record<string,string>, private readonly allowPlaintextRefreshToken=false) {}
  ...
  async get(server:string){ const all=await this.read(server), v=all[server]; if(!v)return undefined; return { ...v, refreshToken:v.refreshToken?await this.crypt("decrypt",v.refreshToken):undefined, clientSecret:v.clientSecret?await this.crypt("decrypt",v.clientSecret):undefined }; }
