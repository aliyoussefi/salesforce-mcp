...
async function discover(endpoint: string): Promise<Discovery> {
  ...
  for (const url of candidates) {
    try {
      ...
      const authorizationServers = Array.isArray(metadata.authorization_servers)
        ? metadata.authorization_servers.filter((v): v is string => typeof v === "string")
        : [];
      ...
      if (issuer && !metadata.authorization_endpoint && !metadata.token_endpoint) {
        for (const suffix of ["/.well-known/openid-configuration", "/.well-known/oauth-authorization-server"]) {
          ...
          if (nested.ok) Object.assign(metadata, await nested.json());
        }
      }
      return {
...
export async function runSetup(file: string): Promise<void> {
  ...
  try {
    ...
    if (!endpoint) throw new Error("An MCP server URL is required.");
    const transport = (await ask(rl, "Transport (streamable-http or sse)", "streamable-http")) as ServerConfig["transport"];
    if (transport !== "streamable-http" && transport !== "sse") throw new Error("Transport must be streamable-http or sse.");
