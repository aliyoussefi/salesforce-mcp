...
const file = arg >= 0 ? process.argv[arg + 1] : undefined;
if (setup) {
...
for (const [name, server] of Object.entries(config.servers)) {
    ...
    if (token)
        await store.set(name, { accessToken: token });
    providers[name] = new OAuthTokenProvider(name, server, store);
}
const tokenProvider = {
    get: async (name) => providers[name]?.get(name),
    set: async (name, tokens) => providers[name]?.set(name, tokens),
};
const router = new ProxyRouter(config, tokenProvider);
const rl = readline.createInterface({ input: process.stdin, crlfDelay: Infinity });
function write(v) { process.stdout.write(JSON.stringify(v) + "\n"); }
rl.on("line", async (line) => { if (!line.trim())
    return; try {
    const rpc = JSON.parse(line);
    if (!rpc.id && rpc.method?.startsWith("notifications/")) {
        await router.handle(rpc);
        return;
    }
    const result = await router.handle(rpc);
    if (result !== undefined)
        write(result);
}
catch (e) {
    ...
    write({ jsonrpc: "2.0", id: rpc.id ?? null, error: { code: -32000, message: e?.message ?? String(e) } });
} });
