...
export class OAuthTokenProvider {
    ...
    async deviceAuthorize() {
        ...
        if (!auth.deviceCodeEndpoint)
            throw new Error(`${this.server} device-code flow requires auth.deviceCodeEndpoint`);
        ...
        if (!response.ok)
            throw new Error(`${this.server} device-code request failed with HTTP ${response.status}: ${await response.text()}`);
        ...
        while (Date.now() < deadline) {
            ...
            if (tokenResponse.ok)
                return this.parseTokenResponse(tokenValue);
            if (tokenValue.error === "authorization_pending")
                continue;
    ...
    async set(server, tokens) {
        if (server === this.server)
            await this.store.set(server, tokens);
    }
    async clientSecret() {
    ...
    async refresh(refreshToken) {
        ...
        if (!response.ok)
            throw new Error(`${this.server} OAuth refresh failed with HTTP ${response.status}: ${await response.text()}`);
    ...
    async authorize() {
        ...
        if (!response.ok)
            throw new Error(`${this.server} OAuth exchange failed with HTTP ${response.status}: ${await response.text()}`);
        return this.parseTokenResponse(await response.json());
    }
    parseTokenResponse(value, previousRefreshToken) {
